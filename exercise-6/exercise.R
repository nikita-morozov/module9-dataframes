...
teams.played.aganist <- husky.games.2015$opponent
...
most <- sort(husky.games.2015$combined,decreasing = TRUE)[1]
print(most)
...
mostYardsScore <- function(games){
  row.number <- order(husky.games.2015$combined,decreasing = TRUE)[1]
  row <- husky.games.2015[row.number,]
}
